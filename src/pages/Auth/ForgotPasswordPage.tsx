...
const ForgotPasswordPage: React.FC = () => {
  ...
  const handleSendOTP = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!email.trim()) {
      showError(t('pages.forgotPassword.emailRequired'));
      return;
    }
    ...
    if (response.success) {
      ...
      showError(response.message || t('pages.forgotPassword.failedToSendOTP'));
    }
    setLoading(false);
  };
  ...
  const handleResetPassword = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!otp.trim()) {
      showError(t('pages.forgotPassword.otpRequired'));
      return;
    }

    if (!newPassword.trim()) {
      showError(t('pages.forgotPassword.passwordRequired'));
      return;
    }

    if (newPassword.length < 8) {
      showError(t('pages.forgotPassword.passwordMinLength'));
      return;
    }

    if (newPassword !== confirmPassword) {
      showError(t('pages.forgotPassword.passwordsDoNotMatch'));
      return;
    }

    setLoading(true);
    const response = await authService.verifyResetPasswordOTP(email, otp, newPassword, confirmPassword);
    if (response.success) {
      showSuccess(response.message);
      setTimeout(() => {
        navigate('/login');
      ...
      showError(response.message || t('pages.forgotPassword.failedToResetPassword'));
    }
    setLoading(false);
  };
  ...
  return (
    <LoginContainer>
      ...
      <FormContainer>
        <FormWrapper onSubmit={otpSent ? handleResetPassword : handleSendOTP}>
          ...
          <Title>{otpSent ? t('pages.forgotPassword.resetPassword') : t('pages.forgotPassword.forgotPassword')}</Title>
          ...
          <InputContainer>
            <input
              ...
              disabled={otpSent}
            />
            <FloatingLabel $hasValue={!!email}>
              {t('pages.forgotPassword.email')}
            </FloatingLabel>
          </InputContainer>

          {/* OTP, New Password, Confirm Password - Only shown after OTP is sent */}
          {otpSent && (
            <>
              {/* OTP Field */}
              <InputContainer>
                <input
                  ...
                  maxLength={4}
                />
                <FloatingLabel $hasValue={!!otp}>
                  {t('pages.forgotPassword.otp')}
                </FloatingLabel>
              </InputContainer>
              ...
              <InputContainer>
                <input
                  ...
                  placeholder=" "
                />
                <FloatingLabel $hasValue={!!newPassword}>
                  {t('pages.forgotPassword.newPassword')}
                </FloatingLabel>
                <PasswordToggle
                  type="button"
              ...
              <InputContainer>
                <input
                  ...
                  placeholder=" "
                />
                <FloatingLabel $hasValue={!!confirmPassword}>
                  {t('pages.forgotPassword.confirmPassword')}
                </FloatingLabel>
                <PasswordToggle
                  type="button"
          ...
          <LoginButton type="submit" disabled={loading}>
            {loading 
              ? (otpSent ? t('pages.forgotPassword.resetting') : t('pages.forgotPassword.sending'))
              : (otpSent ? t('pages.forgotPassword.resetPassword') : t('pages.forgotPassword.sendOTP'))
            }
          </LoginButton>

          {/* Back to Login Link */}
          <FooterText>
            <span>{t('pages.forgotPassword.rememberPassword')} </span>
